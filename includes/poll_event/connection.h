...
namespace cpptools::pollEvent
{
    ...
    public:
        ...
        void setclosecallback(std::function<void(ConnectionPtr)> fn);    // 设置关闭fd_的回调函数。
        void seterrorcallback(std::function<void(ConnectionPtr)> fn);    // 设置fd_发生了错误的回调函数。
        void setonmessagecallback(std::function<void(ConnectionPtr, std::string & )> fn);    // 设置处理报文的回调函数。
        void setsendcompletecallback(std::function<void(ConnectionPtr)> fn);    // 发送数据完成后的回调函数。
        ...
        bool timeout(Time now, int val);           // 判断TCP连接是否超时（空闲太久）。
    };
}
