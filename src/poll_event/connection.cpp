#include "poll_event/connection.h"
...
namespace cpptools::pollEvent
{
              ...
              clientchannel_(new Channel(loop_, clientsock_->getFd()))
    {
        ...
        clientchannel_->setreadcallback([this]
                                        { onmessage(); });
        clientchannel_->setclosecallback([this]
                                         { closecallback(); });
        clientchannel_->seterrorcallback([this]
                                         { errorcallback(); });
    ...
    String Connection::ip() const                   // 返回客户端的ip。
    {
        return clientsock_->getHost();
    }
...
// 设置关闭fd_的回调函数。
    void Connection::setclosecallback(std::function<void(ConnectionPtr)> fn)
    {
        closecallback_ = std::move(fn);     // 回调TcpServer::closeconnection()。
    }

// 设置fd_发生了错误的回调函数。
    void Connection::seterrorcallback(std::function<void(ConnectionPtr)> fn)
    {
        errorcallback_ = std::move(fn);     // 回调TcpServer::errorconnection()。
    }

// 设置处理报文的回调函数。
    void Connection::setonmessagecallback(std::function<void(ConnectionPtr, std::string &)> fn)
    {
        onmessagecallback_ = std::move(fn);       // 回调TcpServer::onmessage()。
    }

// 发送数据完成后的回调函数。
    void Connection::setsendcompletecallback(std::function<void(ConnectionPtr)> fn)
    {
        sendcompletecallback_ = std::move(fn);
    }
        ...
        while (true)             // 由于使用非阻塞IO，一次读取buffer大小数据，直到全部的数据读取完毕。
        {
            ...
            } else if (nread == -1 && ((errno == EAGAIN) || (errno == EWOULDBLOCK))) // 全部的数据已读取完毕。
            {
                ...
                while (true)             // 从接收缓冲区中拆分出客户端的请求消息。
                {
                    ...
                    lastatime_ = Time::now();             // 更新Connection的时间戳。
...
// 处理写事件的回调函数，供Channel回调。
    ...
    // 判断TCP连接是否超时（空闲太久）。
    bool Connection::timeout(Time now, int val)
    {
        return now.unixSecond() - lastatime_.unixSecond() > val;
    }
}
