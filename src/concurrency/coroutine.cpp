...
namespace cpptools::concurrency
{
    ...
    public:
        ...
        explicit Percentile(size_t maxStatDataLen) : max_stat_data_len_(maxStatDataLen)
        {}
        ...
        void Stat(const std::string &key, int64_t value)
        {
            ...
            origin_stat_data_index_[key] = (int32_t) ((origin_stat_data_index_[key] + 1) % max_stat_data_len_);  // 更新下标
        }
        ...
        bool GetPercentile(const std::string &key, double pct, double &pctValue)
        {
            ...
            double x = (double) (temp.size() - 1) * pct;
            auto i = (int32_t) x;
            double j = x - i;
            pctValue = (1 - j) * (double) temp[i] + j * (double) temp[i + 1];
            return true;
        }
    ...
    static bool isBatchDone(Schedule &schedule, int batchId)
    {
        ...
        const auto &cid2finish = schedule.batchs[batchId]->cid2finish;
        return std::all_of(cid2finish.begin(), cid2finish.end(),
                           [](const auto &kv)
                           { return kv.second; });
    }
